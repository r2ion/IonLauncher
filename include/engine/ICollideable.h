...
class ICollideable
{
  public:
	...
	virtual const Vector3D& OBBMins() const = 0; // 1
	virtual const Vector3D& OBBMaxs() const = 0; // 2
	virtual float HitboxTestRadius() const = 0; // 3
	virtual void WorldSpaceTriggerBounds(Vector3D* pWorldMins, Vector3D* pWorldMaxs) const = 0; // 4
	...
	virtual const Vector3D& GetCollisionOrigin() const = 0; // 9
	...
	virtual void WorldSpaceSurroundingBounds(Vector3D* pWorldMins, Vector3D* pWorldMaxs) = 0; // 18
	...
	virtual void MarkPartitionHandleDirty() = 0; // 25
};
