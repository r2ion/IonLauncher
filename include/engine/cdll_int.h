...
class IVEngineClient
{
  public:
    ...
    virtual void SetViewOriginOverride(const Vector3D* pOrigin, int value, int stateId, std::uint32_t sequence,
                                       std::uint32_t activationTick) = 0;                 // 61
    ...
    virtual PlaylistVarOverride* GetPlaylistVarOverride(int overrideIndex) = 0;                   // 84
    ...
    virtual const char* GetCurrentPlaylistGameModeVar(std::uint8_t gameModeIndex, const char* pVariableName, bool includeOverrides) = 0; // 101
    ...
    virtual bool ComputeLighting(const float* pPoint, const float* pNormal, bool clamp, float scale, const float* pModulation,
                                 const float* pLightingOrigin, float* pColor) = 0; // 229
    ...
    virtual void GetNetPacketResponseLatency(int flow, int frameNumber, int* pLatencyMilliseconds, int* pChoke) = 0;                   // 243
    virtual int GetNetPacketHistoryCount() = 0;                                                                                        // 244
    virtual void DebugDrawPhysCollide(const void* pCollide, void* pMaterial, const float* pTransform, const std::uint8_t* pColor) = 0; // 245
    ...
    virtual bool GetSaveGameMetadata(const char* pFileName, void* pMapName, void* pComment, void* pElapsedTime) = 0; // 288
    ...
    virtual void RecordDemoCustomData(std::uint64_t identifier, const void* pData, std::size_t dataSize) = 0;                  // 305
    ...
    virtual bool FormatLocalizedText(const char* pToken, const char* const* pSubstitutions, std::uint32_t substitutionCount, char* pOutput,
    ...
    virtual bool FindPersistentArrayElement(const char* pArrayName, const char* pElementName, std::uintptr_t* pElement) = 0; // 358
    ...
    virtual bool IsPersistentArrayElementValid(const char* pArrayName, const char* pElementName, bool* pIsValid) = 0; // 361
    ...
    virtual bool ClaimClientSidePickup(int pickupType, std::uint32_t first, std::uint32_t second, std::uint32_t third) = 0; // 391
