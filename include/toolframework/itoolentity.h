...
public:
	...
	virtual bool GetLocalPlayerEyePosition(Vector3D& origin, QAngle& angles, float& fov) = 0;
	...
	virtual Vector3D GetAbsOrigin(HTOOLHANDLE handle) = 0;
...
public:
	...
	virtual bool SnapPlayerToPosition(const Vector3D& origin, const QAngle& angles,
		IClientEntity* pClientPlayer = nullptr) = 0;
	virtual bool GetPlayerPosition(Vector3D& origin, QAngle& angles,
		IClientEntity* pClientPlayer = nullptr) = 0;
	...
	virtual bool SetKeyValue(CBaseEntity* pEntity, const char* pField, const Vector3D& value) = 0;
	...
	virtual void ReloadParticleDefinitions(const char* pFileName, const void* pBufferData,
		int length) = 0;
	virtual void MoveEngineViewTo(const Vector3D& position, const QAngle& angles) = 0;
