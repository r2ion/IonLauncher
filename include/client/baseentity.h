...
struct FireBulletsInfo_Client_t
{
    ...
    FireBulletsInfo_Client_t(int shots, const Vector3D& source, const Vector3D& direction, float spread = 0.0f, float distance = 8192.0f,
                             bool primaryAttack = true)
        : m_iShots(shots), m_vecSrc(source), m_vecDirShooting(direction), m_spread(spread), m_flDistance(distance), m_bPrimaryAttack(primaryAttack)
    {
    }
...
class C_BaseEntity : public IClientEntity, public IClientModelRenderable
{
  public:
    ...
    virtual void ModifyFireBulletsDamage(C_TakeDamageInfo* damageInfo) = 0;                                                  // 19
    virtual void MakeTracer(const Vector3D& start, const trace_t& trace, C_BaseEntity* entity, unsigned int tracerType) = 0; // 20
  ...
  public:
    ...
    virtual float GetSoundCodeControllerValue() = 0;                                                                                           // 70
    ...
    virtual bool InLocalTeam() = 0;                                                                                                            // 91
    virtual void ModifyFireBulletsTraceFilter(CBulletsTraceFilter* filter) = 0;                                                                // 92
    ...
    virtual bool IsBaseTrain() const = 0;                                                                                   // 177
    ...
    virtual bool IsAimAssistTarget() = 0;                                                                                   // 197
    virtual bool IsIgnoredByAimAssist() const = 0;                                                                          // 198
    virtual bool AddRagdollToFadeQueue() = 0;                                                                               // 199
    ...
    std::uint64_t GetEFlags() const
    {
        return m_iEFlags;
    }
    bool IsEFlagSet(std::uint64_t flags) const
    {
        return (m_iEFlags & flags) != 0;
    }
    ...
    const Vector3D& GetLocalVelocity() const
    {
    ...
    const Vector3D& GetBaseVelocity() const
    {
    ...
    bool IsPredictionEligible() const
    {
    ...
    const char* GetSignifierName() const
    {
    ...
    unsigned char GetRenderFX() const
    {
        return m_nRenderFX;
    }
    ...
    CClientScriptScope m_ScriptScope; // 0x58
