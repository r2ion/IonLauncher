...
public:
	...
	virtual void GetLightingAtPoint(const Vector3D& origin, Vector3D& tint, float scale) = 0; // 9
	virtual void TraceLine(const Vector3D& start, const Vector3D& end, std::uint32_t contentsMask,
	...
	virtual void SubmitTraceLine(const Vector3D& start, const Vector3D& end, std::uint32_t contentsMask,
	...
	virtual bool IsPointInSolid(const Vector3D& position, int contentsMask) = 0; // 15
	virtual bool MovePointInsideControllingObject(void* pParticles, void* pObject, Vector3D* pPoint) = 0; // 16
	virtual bool IsPointInControllingObjectHitBox(void* pParticles, int controlPoint,
		const Vector3D& position, bool bboxOnly, const char* pHitboxSetName) = 0; // 17
	...
	virtual void GetRandomPointsOnControllingObjectHitBox(void* pParticles, int controlPoint,
		int pointCount, float bboxScale, int attempts, Vector3D* pPoints, const Vector3D& directionBias,
		Vector3D* pRelativePoints, int* pHitboxIndices, int desiredHitbox,
	...
	virtual void GetClosestControllingObjectHitBox(void* pParticles, int controlPoint, int pointCount,
		float bboxScale, const Vector3D* pPoints, Vector3D* pRelativePoints, int* pHitboxIndices,
	...
	virtual void GetControllingObjectOBBox(void* pParticles, int controlPoint,
		Vector3D& mins, Vector3D& maxs) = 0; // 22
	virtual Vector3D GetLocalPlayerPos() = 0; // 23
	virtual void GetLocalPlayerEyeVectors(Vector3D* pForward, Vector3D* pRight, Vector3D* pUp) = 0; // 24
	...
	virtual float GetPixelVisibility(void* pQueryHandle, void* pReserved, const Vector3D& origin,
	...
	virtual void QueueTraceImpact(void* pParticles, void* pUserData, std::uint32_t* pParticleId,
		const Vector3D* pOrigin, int flags, const Vector3D* pDirection, float distance,
		bool option0, bool option1) = 0; // 33
	virtual void NullSub34() = 0; // 34
	virtual void FlushQueuedTraceImpacts() = 0; // 35
	virtual void DebugDrawLine(const Vector3D& origin, const Vector3D& destination, int red, int green,
		int blue, bool noDepthTest, float duration) = 0; // 36
	virtual void DebugDrawText(const Vector3D& origin, const char* pText, int red, int green,
	...
	virtual void UpdateModelRenderState(void* pModel, int flags, const Vector3D* pPosition, void* pState) = 0; // 40
