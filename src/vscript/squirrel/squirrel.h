...
struct SquirrelExecutionResult
{
    ...
    {
        return compileResult != SQRESULT_ERROR && called && callResult != SQRESULT_ERROR;
    }
};
...
#pragma region SQVM func wrappers
    ...
    inline SQRESULT compilebuffer(SQBufferState* bufferState, const SQChar* bufferName = "unnamedbuffer", const SQBool bShouldThrowError = false)
    {
...
class SquirrelManagerManager
{
  public:
    ...
    SquirrelManager* operator[](ScriptContext context)
    {
        return m_pSquirrel[static_cast<int>(context)];
    }
