...
// worlds first practical use of thread_local
thread_local SQCompiler* s_pIfDirectiveCompiler;
...
eSQReturnType SQReturnTypeFromString(const char* pReturnType)
{
    static const std::map<std::string, eSQReturnType> sqReturnTypeNameToString = {
        {"bool", eSQReturnType::Boolean}, {"float", eSQReturnType::Float},   {"vector", eSQReturnType::Vector},
        {"int", eSQReturnType::Integer},  {"entity", eSQReturnType::Entity}, {"string", eSQReturnType::String},
        {"array", eSQReturnType::Arrays}, {"asset", eSQReturnType::Asset},   {"table", eSQReturnType::Table}};
...
void SquirrelManager::VMCreated(CSquirrelVM* newSqvm)
{
    ...
    {
        SquirrelDocumentation::GetInstance().RegisterFunction(m_context, *funcReg);
        spdlog::info("Registering {} function {}", CSquirrelContext::GetName(m_context), funcReg->squirrelFuncName);
...
{
    SquirrelExecutionResult result;
    if (!m_pSQVM || !m_pSQVM->sqvm)
    {
        spdlog::error("Cannot execute code, {} squirrel vm is not initialised", CSquirrelContext::GetName(m_context));
        return result;
    }

    if (!pCode)
    {
        spdlog::error("Cannot execute null {} squirrel code", CSquirrelContext::GetName(m_context));
        return result;
    }

    spdlog::info("Executing {} script code {} ", CSquirrelContext::GetName(m_context), pLogCode ? pLogCode : pCode);

    // NOTE: SQBufferState doesn't strdup pCode!
    SQBufferState bufferState = SQBufferState(pCode);

    result.compileResult = compilebuffer(&bufferState, "console");
    spdlog::info("sq_compilebuffer returned {}", PrintSQRESULT.at(result.compileResult));
    if (result.compileResult == SQRESULT_ERROR)
        return result;

    pushroottable(m_pSQVM->sqvm);
    result.callResult = _call(m_pSQVM->sqvm, 0);
    result.called = true;
    spdlog::info("sq_call returned {}", PrintSQRESULT.at(result.callResult));
    return result;
}
...
{
    SquirrelDocumentation::GetInstance().BeginVM(realContext);
    ...
    spdlog::info("CreateNewVM {} {}", CSquirrelContext::GetName(realContext), (void*)sqvm);
    return sqvm;
}
...
template <ScriptContext context> void __fastcall DestroyVMHook(void* a1, CSquirrelVM* sqvm)
{
    ...
    spdlog::info("DestroyVM {} {}", CSquirrelContext::GetName(realContext), (void*)sqvm);
}

template <ScriptContext context> void (*SQCompileError)(HSQUIRRELVM sqvm, const char* error, const char* file, int line, int column);
template <ScriptContext context> void __fastcall ScriptCompileErrorHook(HSQUIRRELVM sqvm, const char* error, const char* file, int line, int column)
{
...
template <ScriptContext context> bool __fastcall CallScriptInitCallbackHook(void* sqvm, const char* callback)
{
        ...
        for (Mod mod : g_pModManager->m_LoadedMods)
        {
            ...
            for (ModScript script : mod.Scripts)
            {
                for (ModScriptCallback modCallback : script.Callbacks)
                {
                    ...
                    {
                        spdlog::info("Running custom {} script callback \"{}\"", CSquirrelContext::GetName(realContext), modCallback.BeforeCallback);
    ...
    spdlog::info("{} CodeCallback {} called", CSquirrelContext::GetName(realContext), callback);
    if (!bShouldCallCustomCallbacks)
        ...
        for (Mod mod : g_pModManager->m_LoadedMods)
        {
            ...
            for (ModScript script : mod.Scripts)
            {
                for (ModScriptCallback modCallback : script.Callbacks)
                {
                    ...
                    {
                        spdlog::info("Running custom {} script callback \"{}\"", CSquirrelContext::GetName(realContext), modCallback.AfterCallback);
...
ADD_SQFUNC("string", NSGetCurrentModName, "", "Returns the mod name of the script running this function",
...
ADD_SQFUNC("string", NSGetCallingModName, "int depth = 0", "Returns the mod name of the script running this function",
           ScriptContext::UI | ScriptContext::CLIENT | ScriptContext::SERVER)
{
    int depth = g_pSquirrel[context]->getinteger(sqvm, 1);
    if (auto mod = g_pSquirrel[context]->getcallingmod(sqvm, depth); mod == nullptr)
...
ON_DLL_LOAD_RELIESON("server.dll", ServerSquirrel, ConCommand, [](CModule module)
{
    ...
    RegisterConCommand("script", ConCommand_script<ScriptContext::SERVER>, "Executes script code on the server vm",
