...
static void (*s_ParsePlayerSettingsFields)(const void* fieldTable, void* record, void* keyValues, bool required, const char* settingsType,
...
// record addresses without inheriting the engine's fixed capacity.
class CPlayerSettingsServer
{
  public:
    void* Reset();
    char* GetCurrentPose(const char* poseName);
    void LoadRecord(void* keyValues);
    uint32_t Count();
  ...
  private:
    static constexpr size_t RECORD_SIZE = 0x68D0;
    static constexpr size_t POSE_OFFSET = 0x2718;
    static constexpr size_t POSE_SIZE = 0x110;
    static constexpr size_t KEYVALUES_OFFSET = 0x68C8;
    ...
    static constexpr size_t HEALTH_OFFSET = 0x2B58;
    static constexpr size_t HEALTH_PER_SEGMENT_OFFSET = 0x2B5C;
    static constexpr size_t HEALTH_SHIELD_OFFSET = 0x2B60;
    static constexpr size_t HEALTH_DOOMED_OFFSET = 0x2B64;
    ...
    void ResolveModels(std::byte* record) const;
...
CPlayerSettingsServer g_PlayerSettingsServer;

std::byte* CPlayerSettingsServer::At(size_t index) const
{
    return index < m_Records.size() ? m_Records[index]->m_Data.data() : nullptr;
}

template <typename T> T CPlayerSettingsServer::ReadValue(const std::byte* record, size_t offset)
{
    T value;
...
template <typename T> void CPlayerSettingsServer::WriteValue(std::byte* record, size_t offset, const T& value)
{
    std::memcpy(record + offset, &value, sizeof(value));
}

void CPlayerSettingsServer::InitialiseRecord(std::byte* record) const
{
...
int CPlayerSettingsServer::FindIndex(const char* className) const
{
...
void CPlayerSettingsServer::ReleaseRecords()
{
...
int CPlayerSettingsServer::FindPose(const char* name)
{
    if (!name)
        return -1;
...
void CPlayerSettingsServer::ParsePose(std::byte* record, void* keyValues, int pose) const
{
    std::byte* poseRecord = record + POSE_OFFSET + POSE_SIZE * pose;
    s_ParsePlayerSettingsFields(s_pPlayerSettingsPoseFieldTable, poseRecord, keyValues, true, "Player class pose",
...
void CPlayerSettingsServer::ResolveModels(std::byte* record) const
{
...
bool CPlayerSettingsServer::IsStringArgument(HSQUIRRELVM sqvm, size_t argument)
{
    return sqvm && sqvm->_stackOfCurrentFunction && sqvm->_stackOfCurrentFunction[argument]._Type == OT_STRING;
}

int CPlayerSettingsServer::GetIndexArgument(HSQUIRRELVM sqvm) const
{
    const char* className = g_pSquirrel[ScriptContext::SERVER]->getstring(sqvm, 1);
    return FindIndex(className);
}

template <size_t ValueOffset> SQRESULT CPlayerSettingsServer::PushFloat(HSQUIRRELVM sqvm)
{
    ...
    g_pSquirrel[ScriptContext::SERVER]->pushfloat(sqvm, ReadValue<float>(record, ValueOffset));
    return static_cast<SQRESULT>(1);
}

void* CPlayerSettingsServer::Reset()
{
    std::lock_guard<std::recursive_mutex> lock(m_Mutex);
    ReleaseRecords();
...
char* CPlayerSettingsServer::GetCurrentPose(const char* poseName)
{
    std::lock_guard<std::recursive_mutex> lock(m_Mutex);
    const int pose = FindPose(poseName);
    const int index = *s_pCurrentPlayerSettingsIndex;
...
void CPlayerSettingsServer::LoadRecord(void* keyValues)
{
    ...
    *s_pCurrentPlayerSettingsIndex = static_cast<int>(index);
    ...
    ResolveModels(recordData);
    *s_pCurrentPlayerSettingsIndex = -1;
}

uint32_t CPlayerSettingsServer::Count()
{
    std::lock_guard<std::recursive_mutex> lock(m_Mutex);
    return static_cast<uint32_t>(m_Records.size());
}

int CPlayerSettingsServer::NameToIndex(const char* className)
{
    std::lock_guard<std::recursive_mutex> lock(m_Mutex);
    return FindIndex(className);
}

char* CPlayerSettingsServer::GetPose(int index, int pose)
{
    std::lock_guard<std::recursive_mutex> lock(m_Mutex);
    std::byte* record = index >= 0 ? At(static_cast<size_t>(index)) : nullptr;
...
char* CPlayerSettingsServer::GetRecord(int index)
{
    std::lock_guard<std::recursive_mutex> lock(m_Mutex);
    std::byte* record = index >= 0 ? At(static_cast<size_t>(index)) : nullptr;
...
SQRESULT CPlayerSettingsServer::IndexToName(HSQUIRRELVM sqvm)
{
    std::lock_guard<std::recursive_mutex> lock(m_Mutex);
...
const char** CPlayerSettingsServer::GetModelField(const char** result, const char* className, const char* fieldName)
{
    std::lock_guard<std::recursive_mutex> lock(m_Mutex);
...
SQRESULT CPlayerSettingsServer::GetField(HSQUIRRELVM sqvm)
{
    if (!IsStringArgument(sqvm, 1))
    {
        s_PlayerSettingsLog("Classname must be a string");
...
SQRESULT CPlayerSettingsServer::GetHealth(HSQUIRRELVM sqvm)
{
    return PushFloat<HEALTH_OFFSET>(sqvm);
}

SQRESULT CPlayerSettingsServer::GetHealthShield(HSQUIRRELVM sqvm)
{
    return PushFloat<HEALTH_SHIELD_OFFSET>(sqvm);
}

SQRESULT CPlayerSettingsServer::GetHealthDoomed(HSQUIRRELVM sqvm)
{
    return PushFloat<HEALTH_DOOMED_OFFSET>(sqvm);
}

SQRESULT CPlayerSettingsServer::GetHealthPerSegment(HSQUIRRELVM sqvm)
{
    return PushFloat<HEALTH_PER_SEGMENT_OFFSET>(sqvm);
}

DECLARE_HOOK(PlayerSettingsServer_Reset, server.dll + 0x555390, [](auto&) -> void* { return g_PlayerSettingsServer.Reset(); })

DECLARE_HOOK(PlayerSettingsServer_GetCurrentPose, server.dll + 0x555260,
             [](auto&, const char* poseName) -> char* { return g_PlayerSettingsServer.GetCurrentPose(poseName); })

DECLARE_HOOK(PlayerSettingsServer_LoadRecord, server.dll + 0x555560,
             [](auto&, void* keyValues) -> void { g_PlayerSettingsServer.LoadRecord(keyValues); })

DECLARE_HOOK(PlayerSettingsServer_Count, server.dll + 0x555F00, [](auto&) -> uint32_t { return g_PlayerSettingsServer.Count(); })

DECLARE_HOOK(PlayerSettingsServer_NameToIndex, server.dll + 0x555F10,
             [](auto&, const char* className) -> int { return g_PlayerSettingsServer.NameToIndex(className); })

DECLARE_HOOK(PlayerSettingsServer_GetPose, server.dll + 0x556050,
             [](auto&, int index, int pose) -> char* { return g_PlayerSettingsServer.GetPose(index, pose); })

DECLARE_HOOK(PlayerSettingsServer_GetRecord, server.dll + 0x556080, [](auto&, int index) -> char* { return g_PlayerSettingsServer.GetRecord(index); })

DECLARE_HOOK(PlayerSettingsServer_IndexToName, server.dll + 0x552F70,
             [](auto&, HSQUIRRELVM sqvm) -> SQRESULT { return g_PlayerSettingsServer.IndexToName(sqvm); })
...
{ return g_PlayerSettingsServer.GetModelField(result, className, fieldName); })

DECLARE_HOOK(PlayerSettingsServer_GetField, server.dll + 0x556F20,
             [](auto&, HSQUIRRELVM sqvm) -> SQRESULT { return g_PlayerSettingsServer.GetField(sqvm); })

DECLARE_HOOK(PlayerSettingsServer_GetHealth, server.dll + 0x5570B0,
             [](auto&, HSQUIRRELVM sqvm) -> SQRESULT { return g_PlayerSettingsServer.GetHealth(sqvm); })

DECLARE_HOOK(PlayerSettingsServer_GetHealthShield, server.dll + 0x557140,
             [](auto&, HSQUIRRELVM sqvm) -> SQRESULT { return g_PlayerSettingsServer.GetHealthShield(sqvm); })

DECLARE_HOOK(PlayerSettingsServer_GetHealthDoomed, server.dll + 0x5571D0,
             [](auto&, HSQUIRRELVM sqvm) -> SQRESULT { return g_PlayerSettingsServer.GetHealthDoomed(sqvm); })

DECLARE_HOOK(PlayerSettingsServer_GetHealthPerSegment, server.dll + 0x557260,
             [](auto&, HSQUIRRELVM sqvm) -> SQRESULT { return g_PlayerSettingsServer.GetHealthPerSegment(sqvm); })

ON_DLL_LOAD("server.dll", PlayerSettingsServer, [](CModule module)
{
    s_pPlayerSettingsCount = module.Offset(0x14D8338).RCast<int*>();
    s_pCurrentPlayerSettingsIndex = module.Offset(0x14D833C).RCast<int*>();
    s_pDefaultPlayerSettingsIndex = module.Offset(0x14D8340).RCast<int*>();
    s_ReleasePlayerSettingsKeyValues = module.Offset(0x723260).RCast<decltype(s_ReleasePlayerSettingsKeyValues)>();
    s_GetFirstPlayerSettingsKeyValuesChild = module.Offset(0x71D7E0).RCast<decltype(s_GetFirstPlayerSettingsKeyValuesChild)>();
    s_GetNextPlayerSettingsKeyValuesSibling = module.Offset(0x71DA30).RCast<decltype(s_GetNextPlayerSettingsKeyValuesSibling)>();
    s_GetPlayerSettingsKeyValuesName = module.Offset(0x71D9B0).RCast<decltype(s_GetPlayerSettingsKeyValuesName)>();
    s_FindPlayerSettingsKeyValuesChild = module.Offset(0x71D120).RCast<decltype(s_FindPlayerSettingsKeyValuesChild)>();
    s_ParsePlayerSettingsFields = module.Offset(0x651FA0).RCast<decltype(s_ParsePlayerSettingsFields)>();
    s_ParsePlayerSettingsGlobal = module.Offset(0x555810).RCast<decltype(s_ParsePlayerSettingsGlobal)>();
    s_ParsePlayerSettingsModifier = module.Offset(0x5542F0).RCast<decltype(s_ParsePlayerSettingsModifier)>();
    s_ApplyPlayerSettingsModifiers = module.Offset(0x554520).RCast<decltype(s_ApplyPlayerSettingsModifiers)>();
    s_ParsePlayerSettingsPoseMods = module.Offset(0x554930).RCast<decltype(s_ParsePlayerSettingsPoseMods)>();
    s_ResolvePlayerSettingsModel = module.Offset(0x159C00).RCast<decltype(s_ResolvePlayerSettingsModel)>();
    s_FindPlayerSettingsField = module.Offset(0x651F20).RCast<decltype(s_FindPlayerSettingsField)>();
    s_PushPlayerSettingsField = module.Offset(0x556300).RCast<decltype(s_PushPlayerSettingsField)>();
    s_PushPlayerSettingsGlobalField = module.Offset(0x555E70).RCast<decltype(s_PushPlayerSettingsGlobalField)>();
    s_GetSquirrelArgumentCount = module.Offset(0x6F30).RCast<decltype(s_GetSquirrelArgumentCount)>();
    s_GetSquirrelInteger = module.Offset(0x5D30).RCast<decltype(s_GetSquirrelInteger)>();
    s_PlayerSettingsLog = module.Offset(0x2A8750).RCast<decltype(s_PlayerSettingsLog)>();
    s_pPlayerSettingsGlobalFieldTable = module.Offset(0xB80FF0).RCast<const void*>();
    s_pPlayerSettingsSecondaryFieldTable = module.Offset(0xB85AC0).RCast<const void*>();
    s_pPlayerSettingsPoseFieldTable = module.Offset(0xB85C40).RCast<const void*>();
