...
template <typename Key> struct RHashMap
{
    ...
    bool (*keysEqual)(const void* entry, Key key);
    ...
    void* Find(Key key) const
    {
        uint32_t hashOrBucket = hashKey(key);
        return FindWithHash(hashOrBucket, key);
    }

    void* FindOrReserveUnlocked(Key key, bool& reserved)
    {
        uint32_t hashOrBucket = hashKey(key);
    ...
    uint32_t* RemoveExisting(Key key)
    {
        void* entry = Find(key);
  ...
  private:
    ...
    void* FindWithHash(uint32_t& hashOrBucket, Key key) const
    {
    ...
    uint32_t* RemoveEntry(void* entry, Key key)
    {
        const uint32_t bucketMask = BucketCount() - 1;
...
using RHashMapU32 = RHashMap<std::uint32_t>;
using RHashMapString = RHashMap<const char*>;
...
struct RBitRead
{
    ...
    FORCEINLINE int BitsAvailable() const
    {
        return m_bitsAvailable;
    };
