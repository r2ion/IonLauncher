                            ...
                            const __m128* clipThreshold, const __m128* uvBias, const __m128* viewportScale)
{
    ...
    auto SubmitDraw = [&]() -> bool
    {
        ...
        return s_RuiDrawInfoHandlers[static_cast<uint32_t>(drawInfo->mode)](drawInfo, &drawUv, &quad, batch);
    };
    ...
    auto DrawPiece = [&](__m128 u, __m128 v, const __m128* correction, bool useAlternateOrientationShuffle, __m128 base, __m128 xDirection,
                         __m128 yDirection) -> bool
    {
        SetTriangleFromUv(u, v, correction, useAlternateOrientationShuffle, false);
        drawUv.primaryOrigin = yDirection;
        drawUv.primaryBasisX = base;
        drawUv.primaryBasisY = xDirection;
    ...
    if (textureOffsetIndex >= imageAtlas->nineSliceImageCount)
    {
        ...
        __m128 projected[2];
        projected[0] = _mm_add_ps(_mm_add_ps(_mm_mul_ps(RUI_SHUFFLE_PS(transformRow0, 170), u), _mm_mul_ps(RUI_SHUFFLE_PS(transformRow0, 0), v)),
                                  RUI_SHUFFLE_PS(transform->rows[1], 0));
        projected[1] = _mm_add_ps(_mm_add_ps(_mm_mul_ps(RUI_SHUFFLE_PS(transformRow0, 255), u), _mm_mul_ps(RUI_SHUFFLE_PS(transformRow0, 85), v)),
        ...
        RuiDrawInfo* drawInfo = rui->drawInfo;
        return s_RuiDrawInfoHandlers[static_cast<uint32_t>(drawInfo->mode)](drawInfo, baseUv, &quad, batch);
    }
    ...
    const __m128 refinedReciprocalScale = _mm_add_ps(
        _mm_mul_ps(_mm_add_ps(_mm_mul_ps(reciprocalScaleError, reciprocalScaleError), reciprocalScaleError), reciprocalScale), reciprocalScale);
    ...
    const __m128 edgeSize = _mm_max_ps(_mm_mul_ps(_mm_sqrt_ps(_mm_add_ps(RUI_SHUFFLE_PS(projectedCanvasSq, 78), projectedCanvasSq)),
    ...
    const __m128 refinedAvailableExtentReciprocal =
        _mm_add_ps(_mm_mul_ps(_mm_add_ps(_mm_mul_ps(availableExtentError, availableExtentError), availableExtentError), availableExtentReciprocal),
    ...
    const __m128 innerScale = _mm_movelh_ps(_mm_mul_ps(_mm_mul_ps(inverseAtlasExtent, edgeSize), refinedAvailableExtentReciprocal), s_RuiFloatOne);
    ...
    const __m128 innerYDir = _mm_add_ps(_mm_sub_ps(atlasMin, _mm_mul_ps(_mm_mul_ps(atlasMin, inverseAtlasExtent), refinedAvailableExtentReciprocal)),
                                        _mm_mul_ps(innerScale, baseUv->primaryOrigin));

    const __m128 atlasStep = _mm_mul_ps(
        _mm_sub_ps(_mm_add_ps(_mm_mul_ps(RUI_SHUFFLE_PS(refinedAvailableExtentReciprocal, 238), flippedAtlasRect), s_RuiHighHalfOne), *uvBias),
    ...
    auto BlendByMask = [](__m128 keep, __m128 replace, __m128 mask) { return _mm_or_ps(_mm_andnot_ps(mask, keep), _mm_and_ps(replace, mask)); };

    const int clipMaskY_5 = clipMaskY & 5;
    const int clipMaskY_A = clipMaskY & 0xA;

    if ((clipMaskX & 3) == 0 && !DrawPiece(RUI_SHUFFLE_PS(uvHigh, 20), RUI_SHUFFLE_PS(uvLow, 80), &s_RuiEdgeCorrectionMasks[correctionMask & 5],
                                           false, outerBase, outerXDir, outerYDir))
    {
        return false;
    }

    if ((clipMaskY_5 | (clipMaskX & 2)) ||
        DrawPiece(RUI_SHUFFLE_PS(uvHigh, 20), RUI_SHUFFLE_PS(uvLow, 245), &s_RuiEdgeCorrectionMasks[correctionMask & 4], true,
                  BlendByMask(outerBase, innerBase, s_RuiBlendMaskLane0), BlendByMask(outerXDir, innerXDir, s_RuiBlendMaskLane0),
                  BlendByMask(outerYDir, innerYDir, s_RuiBlendMaskLane0)))
    {
        ...
        if ((clipMaskX & 6) == 0)
        {
            if (!DrawPiece(RUI_SHUFFLE_PS(uvHigh, 20), RUI_SHUFFLE_PS(uvLow, 175), &s_RuiEdgeCorrectionMasks[correctionMask & 6], false, outerBase,
                           outerXDir, BlendByMask(outerYDir, edgeYDir, s_RuiBlendMaskLane0)))
            {
                return false;
            }
        }
        ...
        if ((clipMaskY_A | (clipMaskX & 1)) ||
            DrawPiece(RUI_SHUFFLE_PS(uvHigh, 125), RUI_SHUFFLE_PS(uvLow, 80), &s_RuiEdgeCorrectionMasks[correctionMask & 1], false,
                      BlendByMask(outerBase, innerBase, s_RuiBlendMaskLane1), BlendByMask(outerXDir, innerXDir, s_RuiBlendMaskLane1),
                      BlendByMask(outerYDir, innerYDir, s_RuiBlendMaskLane1)))

        {
            if (clipMaskY || DrawPiece(RUI_SHUFFLE_PS(uvHigh, 125), RUI_SHUFFLE_PS(uvLow, 245), nullptr, true, innerBase, innerXDir, innerYDir))
            {
                if ((clipMaskY_A | (clipMaskX & 4)) ||
                    DrawPiece(RUI_SHUFFLE_PS(uvHigh, 125), RUI_SHUFFLE_PS(uvLow, 175), &s_RuiEdgeCorrectionMasks[correctionMask & 2], false,
                              BlendByMask(outerBase, innerBase, s_RuiBlendMaskLane1), BlendByMask(outerXDir, innerXDir, s_RuiBlendMaskLane1),
                              BlendByMask(edgeYDir, innerYDir, s_RuiBlendMaskLane1)))
                {
                    if ((clipMaskX & 9) == 0 &&
                        !DrawPiece(RUI_SHUFFLE_PS(uvHigh, 235), RUI_SHUFFLE_PS(uvLow, 80), &s_RuiEdgeCorrectionMasks[correctionMask & 9], true,
                                   outerBase, outerXDir, BlendByMask(outerYDir, edgeYDir, s_RuiBlendMaskLane1)))
                    {
                        return false;
                    }

                    if ((clipMaskY_5 | (clipMaskX & 8)) ||
                        DrawPiece(RUI_SHUFFLE_PS(uvHigh, 235), RUI_SHUFFLE_PS(uvLow, 245), &s_RuiEdgeCorrectionMasks[correctionMask & 8], false,
                                  BlendByMask(outerBase, innerBase, s_RuiBlendMaskLane0), BlendByMask(outerXDir, innerXDir, s_RuiBlendMaskLane0),
                                  BlendByMask(edgeYDir, innerYDir, s_RuiBlendMaskLane0)))
                    {
                        if ((clipMaskX & 0xC) == 0)
                        {
                            return DrawPiece(RUI_SHUFFLE_PS(uvHigh, 235), RUI_SHUFFLE_PS(uvLow, 175), &s_RuiEdgeCorrectionMasks[correctionMask & 0xA],
                                             true, outerBase, outerXDir, edgeYDir);
                        }
                    }
                    else
                    {
...
DECLARE_HOOK(RuiDrawImageAtlasEntry, engine.dll + 0xF9B80,
             [](auto& hook, RuiGlobalState* globalState, RuiInstance* rui, RuiDrawBatch* batch, const RuiBaseUv* baseUv,
                const RuiTransform* transform, int orientation, const RuiImageAssetDescriptor* descriptor, const __m128i* atlasUv,
                const __m128* clipThreshold, const __m128* uvBias, const __m128* viewportScale) -> bool
{
    return RuiDrawImageAtlasEntry(globalState, rui, batch, baseUv, transform, orientation, descriptor, atlasUv, clipThreshold, uvBias, viewportScale);
});

static bool RuiRenderEllipseJob(RuiRenderContext* context, RuiInstance* rui, const RuiEllipseRenderJob* job, RuiDrawBatch* batch)
{
    (void)context;
    ...
    auto DataScalar = [&](uint16_t offset) -> __m128 { return _mm_set_ss(DataFloat(offset)); };
    ...
    const __m128 determinant = _mm_sub_ps(_mm_mul_ps(RUI_SHUFFLE_PS(transformRow0, 255), RUI_SHUFFLE_PS(transformRow0, 0)),
    ...
    if (!imageAtlas || !imageAtlas->images || assetIndex < 0 || static_cast<uint16_t>(assetIndex) >= imageAtlas->imageCount)
    {
        return true;
    }
    ...
    const __m128 normalizedTextureBounds =
        _mm_div_ps(_mm_add_ps(_mm_sub_ps(textureBounds, _mm_xor_ps(_mm_and_ps(_mm_min_ps(texMins, texMaxs), axisMask), s_RuiSignMaskLowHalf)),
                              stretchCorrection),
                   _mm_or_ps(_mm_and_ps(_mm_andnot_ps(s_RuiSignMaskAll, textureExtent), axisMask), _mm_andnot_ps(axisMask, s_RuiFloatOne)));
    ...
    const __m128 clippedBounds = _mm_xor_ps(_mm_min_ps(requestedBounds, normalizedTextureBounds), s_RuiSignMaskLowHalf);
    ...
    const __m128 textureBase = _mm_add_ps(_mm_mul_ps(_mm_add_ps(_mm_mul_ps(originSum, textureExtent), texMins), textureScale),
    ...
    RuiDrawInfo* drawInfo = rui->drawInfo;
    return s_RuiDrawInfoHandlers[static_cast<uint32_t>(drawInfo->mode)](drawInfo, &uv, &triangle, batch);
}

static bool RuiRenderImageJob(RuiRenderContext* context, RuiInstance* rui, const RuiImageRenderJob* job, RuiDrawBatch* batch)
{
    ...
    auto DataInt = [&](int offset) -> int32_t
    {
        int32_t value;
        std::memcpy(&value, &rui->data[offset], sizeof(value));
        return value;
    };
    auto DataScalar = [&](int offset) -> __m128 { return _mm_set_ss(DataFloat(offset)); };
    ...
    const __m128 determinant =
        _mm_sub_ps(_mm_mul_ps(RUI_SHUFFLE_PS(transformRow0, _MM_SHUFFLE(3, 3, 3, 3)), RUI_SHUFFLE_PS(transformRow0, _MM_SHUFFLE(0, 0, 0, 0))),
    ...
    if (!imageAtlas || !imageAtlas->images || static_cast<uint16_t>(assetIndex) >= imageAtlas->imageCount ||
        (secondaryAssetIndex != -1 && (secondaryAssetIndex < 0 || static_cast<uint16_t>(secondaryAssetIndex) >= imageAtlas->imageCount)))
    {
        return true;
    }
    ...
    const __m128 normalizedTextureOffset =
        _mm_div_ps(_mm_sub_ps(primaryTextureOffset, _mm_xor_ps(_mm_and_ps(_mm_min_ps(texMins, texMaxs), axisMask), s_RuiSignMaskLowHalf)),
                   _mm_or_ps(_mm_and_ps(_mm_andnot_ps(s_RuiSignMaskAll, textureExtent), axisMask), _mm_andnot_ps(axisMask, s_RuiFloatOne)));
    ...
    const __m128 primaryBase = _mm_add_ps(_mm_mul_ps(textureScale, texturePosition),
    ...
    if (secondaryAssetIndex == -1)
    {
        ...
        const __m128 refinedSpanReciprocal =
            _mm_add_ps(_mm_mul_ps(_mm_add_ps(_mm_mul_ps(spanError, spanError), spanError), spanReciprocal), spanReciprocal);
    ...
    else
    {
        ...
        const __m128 rotationTurns =
            _mm_mul_ps(_mm_add_ps(_mm_xor_ps(RUI_SHUFFLE_PS(maskRotation, 0), s_RuiSignMaskLane2), s_RuiQuarterEndpoints), s_RuiFloatFour);
        const __m128i rotationQuadrant = _mm_cvtps_epi32(rotationTurns);
        const __m128 quadrantIsEven =
            _mm_castsi128_ps(_mm_cmpeq_epi32(_mm_and_si128(_mm_castps_si128(s_RuiIntOne), rotationQuadrant), _mm_setzero_si128()));
        ...
        const __m128 cosApprox = _mm_sub_ps(
            ...
            _mm_sub_ps(
                fractionSq,
                _mm_mul_ps(_mm_add_ps(_mm_mul_ps(_mm_add_ps(_mm_mul_ps(_mm_add_ps(_mm_mul_ps(s_RuiCosApproxCoeff3, fractionSq), s_RuiCosApproxCoeff2),
                                                                       fractionSq),
        ...
        const __m128 sinApprox = _mm_add_ps(
            _mm_mul_ps(_mm_add_ps(_mm_mul_ps(_mm_add_ps(_mm_mul_ps(_mm_add_ps(_mm_mul_ps(s_RuiSinApproxCoeff3, fractionSq), s_RuiSinApproxCoeff2),
                                                                   fractionSq),
                       ...
                       rotationFraction),
            rotationFraction);
        const __m128 quadrantSign = _mm_castsi128_ps(_mm_slli_epi32(_mm_and_si128(_mm_castps_si128(s_RuiIntTwo), rotationQuadrant), 0x1E));
        const __m128 rotationBasis =
            _mm_mul_ps(_mm_xor_ps(_mm_or_ps(_mm_andnot_ps(quadrantIsEven, cosApprox), _mm_and_ps(sinApprox, quadrantIsEven)), quadrantSign),
        ...
        const __m128 maskTexturePosition =
            _mm_mul_ps(_mm_add_ps(_mm_add_ps(maskTranslate, maskTextureCenter), _mm_add_ps(RUI_SHUFFLE_PS(rotatedPosition, 78), rotatedPosition)),
                       maskTextureScale);
        maskBasis = _mm_mul_ps(_mm_add_ps(_mm_mul_ps(RUI_SHUFFLE_PS(rotationBasis, 78), RUI_SHUFFLE_PS(basisExtent, 165)),
                                          _mm_mul_ps(RUI_SHUFFLE_PS(basisExtent, 240), rotationBasis)),
                               maskTextureScale);
        maskBase =
            _mm_add_ps(maskTexturePosition, _mm_castsi128_ps(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(secondaryTextureRecord.uvBase))));
    }
