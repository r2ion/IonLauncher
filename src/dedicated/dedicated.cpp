...
class DedicatedConsoleServerPresence : public ServerPresenceReporter
{
    ...
    {
        SetConsoleTitleA(fmt::format("{} - {} {}/{} players ({})", pServerPresence->m_sServerName, pServerPresence->m_MapName,
                                     pServerPresence->m_iPlayerCount, pServerPresence->m_iMaxPlayers, pServerPresence->m_PlaylistName)
                             .c_str());
    }
};
...
ON_DLL_LOAD_DEDI("server.dll", DedicatedServerGameDLL, [](CModule module)
{
    ...
    if (CommandLine()->CheckParm("-nopakdedi"))
    {
        ...
        module.Offset(0x6BA300).Patch("B8 C8 00 00 00 C3"); // return 200 as the number of skins from server.dll + 6BA300, this is the normal value
                                                            // read from skins.rson and should be updated when we need it more modular
    }
})
