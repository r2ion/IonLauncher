...
{
#ifdef IS_WINDOWS_PC
	AssertMsg( sizeof( UUID ) == sizeof( *pDest ), "UniqueId_t must match UUID" );
...
{
	AssertMsg( pDest, "Unique ID destination must not be null" );
...
bool Unserialize( CUtlBuffer &buf, UniqueId_t &dest )
{
	if ( buf.IsText() )
	{
		int nTextLen = static_cast<int>( buf.PeekStringLength() );
