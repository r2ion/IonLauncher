...
static ConVar* Cvar_rui_drawEnable;
...
static CMemory s_PilotSpeedometerVisibilityBranch;

static fltx4 MakeColor(float red, float green, float blue, float alpha = 1.0f)
{
    return _mm_setr_ps(red, green, blue, alpha);
}

static fltx4 FriendlyTeamColor()
{
    return MakeColor(0.095f, 0.309f, 0.708f);
}

static fltx4 EnemyTeamColor()
{
    return MakeColor(1.0f, 0.188f, 0.014f);
}

static fltx4 RainbowColor(float time, float speed, float hueOffset)
{
    ...
    {
    case 0:
        return MakeColor(1.0f, rising, 0.0f);
    case 1:
        return MakeColor(falling, 1.0f, 0.0f);
    case 2:
        return MakeColor(0.0f, 1.0f, rising);
    case 3:
        return MakeColor(0.0f, falling, 1.0f);
    case 4:
        return MakeColor(rising, 0.0f, 1.0f);
    default:
        return MakeColor(1.0f, 0.0f, falling);
    }
}

static void StoreColor(float (&destination)[4], fltx4 color)
{
    _mm_storeu_ps(destination, color);
}

static void StoreRgb(float (&destination)[3], fltx4 color)
{
...
static void UpdateGamestateInfoColors(RuiGlobalState* globals, float (&left)[3], float (&right)[3])
{
    ...
    {
        StoreRgb(left, RainbowColor(globals->gameTime, 0.5f, 0.0f));
        StoreRgb(right, RainbowColor(globals->gameTime, 0.5f, 0.5f));
        return;
    }
...
static void UpdateGamestateInfoFfa(const RuiFunctionTable_t* api, RuiGlobalState* globals, RuiInstance* rui, RuiGamestateInfoFfaInstanceData* data)
{
    const float timeRemaining = data->endTime - globals->gameTime;
    if (timeRemaining < 0.0f || data->endTime == -1.0e30f)
        data->formattedTime = "--:--";
    else if (timeRemaining > 30.0f)
        data->formattedTime =
            api->format(rui, "%i:%02i", static_cast<unsigned int>(timeRemaining) / 60, static_cast<unsigned int>(timeRemaining) % 60);
    else
    ...
    {
        api->setErrorWithReason(rui, data->leftTeamScore < data->rightTeamScore
    ...
    if (Cvar_ion_chroma_gameinfo->GetBool())
    {
        topColor = RainbowColor(globals->gameTime, 0.5f, 0.0f);
        bottomColor = RainbowColor(globals->gameTime, 0.5f, 0.5f);
    }
    ...
    const char* localizationKey = data->statusText && data->statusText[0] ? data->statusText : "#PL_ffa";
    ...
    fltx4* transformSizes = api->getTransformSizes(rui);
...
static void UpdateCrosshairPlus(const RuiFunctionTable_t* api, RuiGlobalState* globals, RuiInstance* rui, RuiCrosshairPlusInstanceData* data)
{
    if (globals->hasOpenMenu)
    {
        api->setNoRender(rui);
        return;
    }
    ...
    fltx4 movX = _mm_set_ss(data->movementX);
    fltx4 movY = _mm_set_ss(data->movementY);
    fltx4 movXY = _mm_unpacklo_ps(movX, movY);

    // Packs (movX + offX, movY + offY) into a uint64 for qword fields
    // offX and offY are passed as fltx4 scalars (raw hex bits preserved)
...
static void UpdateKraberAmmoCounter(const RuiFunctionTable_t* api, RuiGlobalState* globals, RuiInstance* rui, RuiKraberAmmoCounterInstanceData* data)
{
    data->ammoFraction = static_cast<float>(data->ammo) / std::max(static_cast<float>(data->clipSize), 1.0f);
    const AmmoUrgency urgency = ComputeAmmoUrgency(data->ammoFraction, globals->gameTime);
    ...
    data->scrollPosition = globals->gameTime * 10.0f;
    ...
    const fltx4 pulsedColor =
        _mm_mul_ps(_mm_mul_ps(_mm_set1_ps(NoiseEnvelope(globals->gameTime)), urgency.color), _mm_set1_ps(api->randomFloat(rui) * 0.15000001f));
...
static void UpdateMastiffAmmoCounter(const RuiFunctionTable_t* api, RuiGlobalState* globals, RuiInstance* rui,
...
template <typename T> static bool WriteProtectedValue(T* address, const T& value)
{
    DWORD oldProtection;
...
static void UpdateGauntletSpeedLimits()
{
    ...
    if (requestedMaxSpeedMph != appliedMaxSpeedMph && WriteProtectedValue(s_GauntletMaxSpeedMph, requestedMaxSpeedMph))
    {
        appliedMaxSpeedMph = requestedMaxSpeedMph;
    }

    const float requestedMaxSpeedKph = Cvar_gauntlet_timer_max_speed_metric->GetFloat();
    if (requestedMaxSpeedKph != appliedMaxSpeedKph && WriteProtectedValue(s_GauntletMaxSpeedKph, requestedMaxSpeedKph))
    {
...
DECLARE_HOOK(gamestate_info_ffa, ui(11).dll + 0x3E8E0,
             [](auto& hook, const RuiFunctionTable_t* api, RuiGlobalState* globals, RuiInstance* rui, RuiGamestateInfoFfaInstanceData* data)
{ UpdateGamestateInfoFfa(api, globals, rui, data); })

DECLARE_HOOK(gamestate_info, ui(11).dll + 0x33AE0,
             [](auto& hook, const RuiFunctionTable_t* api, RuiGlobalState* globals, RuiInstance* rui, RuiGamestateInfoInputPrefix* data)
{
    UpdateGamestateInfoColors(globals, data->leftColor, data->rightColor);
    hook.Original(api, globals, rui, data);
})

DECLARE_HOOK(gamestate_info_ps, ui(11).dll + 0x46280,
             [](auto& hook, const RuiFunctionTable_t* api, RuiGlobalState* globals, RuiInstance* rui, RuiGamestateInfoPsInstanceData* data)
{
    UpdateGamestateInfoColors(globals, data->leftColor, data->rightColor);
    hook.Original(api, globals, rui, data);
})

DECLARE_HOOK(crosshair_plus, ui(11).dll + 0x1D560,
             [](auto& hook, const RuiFunctionTable_t* api, RuiGlobalState* globals, RuiInstance* rui, RuiCrosshairPlusInstanceData* data)
{
...
DECLARE_HOOK(kraber_ammo_counter, ui(11).dll + 0x58BC0,
             [](auto& hook, const RuiFunctionTable_t* api, RuiGlobalState* globals, RuiInstance* rui, RuiKraberAmmoCounterInstanceData* data)
{ UpdateKraberAmmoCounter(api, globals, rui, data); })

DECLARE_HOOK(mastiff_ammo_counter, ui(11).dll + 0x5D710,
             [](auto& hook, const RuiFunctionTable_t* api, RuiGlobalState* globals, RuiInstance* rui, RuiMastiffAmmoCounterInstanceData* data)
{ UpdateMastiffAmmoCounter(api, globals, rui, data); })

DECLARE_HOOK(pilot_speedometer, ui(11).dll + 0x6AA50,
             [](auto& hook, const RuiFunctionTable_t* api, RuiGlobalState* globals, RuiInstance* rui, RuiPilotSpeedometerInstanceData* data)
{
    ...
    {
        if (shouldAlwaysShow)
            s_PilotSpeedometerVisibilityBranch.Patch({0x90, 0x90});
        else
            s_PilotSpeedometerVisibilityBranch.Patch({0x72, 0x2C});
...
DECLARE_HOOK(gauntlet_hud, ui(11).dll + 0x4E030,
             [](auto& hook, const RuiFunctionTable_t* api, RuiGlobalState* globals, RuiInstance* rui, RuiGauntletHudInstanceData* data)
{
...
ON_DLL_LOAD_CLIENT_RELIESON("engine.dll", RUI, ConVar, [](CModule module)
{
    ...
    Cvar_ion_use_custom_crosshair =
        new ConVar("ion_use_custom_crosshair", "0", FCVAR_ARCHIVE_PLAYERPROFILE, "Use custom crosshairs. 1 = enabled, 0 = disabled.");
    ...
    Cvar_ion_crosshair_thickness_l =
        new ConVar("ion_crosshair_thickness_l", "0.00185", FCVAR_ARCHIVE_PLAYERPROFILE, "Vertical arm thickness left of center.");
    Cvar_ion_crosshair_thickness_r =
        new ConVar("ion_crosshair_thickness_r", "0.00833", FCVAR_ARCHIVE_PLAYERPROFILE, "Vertical arm thickness right of center.");
    ...
    Cvar_gauntlet_timer_max_speed_metric =
        new ConVar("gauntlet_timer_max_speed_metric", "50.05047", FCVAR_ARCHIVE_PLAYERPROFILE, "Max speed in gauntlet timer (metric).");
    Cvar_gauntlet_timer_max_speed_imperial =
        new ConVar("gauntlet_timer_max_speed_imperial", "31.1", FCVAR_ARCHIVE_PLAYERPROFILE, "Max speed in gauntlet timer (imperial).");
})

ON_DLL_LOAD_CLIENT_RELIESON("ui(11).dll", RuiUiCallbacks, RUI, [](CModule module)
{
    s_PilotSpeedometerVisibilityBranch = module.Offset(PILOT_SPEEDOMETER_VISIBILITY_BRANCH);
